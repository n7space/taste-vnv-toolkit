...
if not taste_project_directory:
    ...
    if not report_path:
        status = "error"
        status_text = (
            "Cppcheck HTML report not found. Run the tool first to generate it."
        )
        show_status = True
    else:
        try:
            ...
            status = "ok"
            status_text = f"Opened Cppcheck HTML report: {report_path}"
            show_status = False
        except Exception as exc:
            status = "error"
            status_text = f"Failed to open Cppcheck HTML report: {exc}"
            show_status = True
